...
namespace simox::alg
{
    ...
    bool
    contains(const ContainerT& container, const ValueT& value)
    {
        return std::find_if(container.begin(),
                            container.end(),
                            [&value](const auto& v) { return v == value; }) != container.end();
    }
    ...
    bool
    contains(const ContainerT& container, const ValueT& value, const PredicateT& predicate)
    {
        return std::find_if(container.begin(),
                            container.end(),
                            [&value, &predicate](const auto& v)
                            { return predicate(v, value); }) != container.end();
    }
    ...
    bool
    contains_key(const MapT<K, V, Ts...>& map, const K& key)
    {
    ...
    template <class K, class V, template <class...> class MapT = std::map, class... Ts>
    bool
    contains_value(const MapT<K, V, Ts...>& map, const V& value)
    {
        return std::find_if(map.begin(),
                            map.end(),
                            [&value](const std::pair<K, V>& item)
                            { return item.second == value; }) != map.end();
    }
    ...
    bool
    contains_key(const MapT<std::string, V, Ts...>& map, const std::string& key)
    {
        return map.count(key) > 0;
    }
    ...
    bool
    contains_value(const MapT<K, std::string, Ts...>& map, const std::string& value)
    {
