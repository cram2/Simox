...
namespace simox
{
    ...
    public:
        OrientedBoxBase(const transform_t& t, const vector_t& d) : _t{t}, _d{d}
        {
        }
    ...
    protected:
        static auto
        translation(transform_t& t)
        {
            return t.template topRightCorner<3, 1>();
        }

        static auto
        translation(const transform_t& t)
        {
            return t.template topRightCorner<3, 1>();
        }

        static auto
        rotation(transform_t& t)
        {
            return t.template topLeftCorner<3, 3>();
        }

        static auto
        rotation(const transform_t& t)
        {
            return t.template topLeftCorner<3, 3>();
        }
        ...
        static transform_t
        transformation(const rotation_t& rot, const vector_t& trans)
        {
    ...
    public:
        ...
        void
        scale(const vector_t& factors)
        {
            _d = _d.cwiseProduct(factors);
        }

        void
        scale(float_t factor)
        {
            _d *= factor;
        }

        void
        scale_centered(float_t factor)
        {
        ...
        vector_t
        from_box_frame(const vector_t& p) const
        {
        ...
        vector_t
        to_box_frame(const vector_t& p) const
        {
        ...
        bool
        contains(const vector_t& p) const
        {
            const vector_t b = to_box_frame(p);
            const auto check_dim = [&](int i)
            { return _d(i) < 0 ? (b(i) <= 0 && b(i) >= _d(i)) : (b(i) >= 0 && b(i) <= _d(i)); };
            return check_dim(0) && check_dim(1) && check_dim(2);
        }

        bool
        contains_by(const vector_t& p, float_t thresh = 0) const
        {
            const vector_t b = to_box_frame(p);
            const auto check_dim = [&](int i)
            {
                return _d(i) < 0 ? (b(i) <= -thresh && b(i) >= _d(i) + thresh)
        ...
        std::vector<Eigen::Matrix<T, 3, 1>, Other...>
        contained_points(const std::vector<Eigen::Matrix<T, 3, 1>, Other...>& ps)
        {
        ...
        corner(std::uint8_t c) const
        {
            ...
            const vector_t b{
                (c % 2) ? 0 : _d(0), ((c / 2) % 2) ? 0 : _d(1), ((c / 4) % 2) ? 0 : _d(2)};
