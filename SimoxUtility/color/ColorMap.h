...
namespace simox::color
{
    ...
    public:
        ...
        void
        add_key(float value, const Color& color)
        {
            keys[value] = color;
        }
        ...
        template <typename V>
        std::vector<Color>
        operator()(const std::vector<V>& vector) const
        {
        ...
        std::map<K, Color>
        operator()(const std::map<K, V>& map) const
        {
        ...
        void
        set_vmin(const std::vector<float>& values)
        {
            set_vmin(*std::max_element(values.begin(), values.end()));
        }

        void
        set_vmax(const std::vector<float>& values)
        {
        ...
        template <class T>
        void
        set_vmin(const std::vector<T>& values, std::function<float(const T&)> unaryFunc)
        {
        ...
        template <class T>
        void
        set_vmax(const std::vector<T>& values, std::function<float(const T&)> unaryFunc)
        {
        ...
        void
        set_vlimits(float vmin, float vmax)
        {
            set_vmin(vmin);
            set_vmax(vmax);
        }
        ...
        void
        set_vlimits(const std::vector<float>& values)
        {
        ...
        template <class T>
        void
        set_vlimits(const std::vector<T>& values, std::function<float(const T&)> unaryFunc)
        {
