...
BOOST_AUTO_TEST_CASE(testWorkSpaceGrid)
{
    ...
    m2(2, 3) = m(2, 3);
    VirtualRobot::WorkspaceRepresentation::WorkspaceCut2DPtr cutXY =
        reach->createCut(m2, discrTr, false);
    BOOST_REQUIRE(cutXY);
    ...
    std::vector<VirtualRobot::WorkspaceRepresentation::WorkspaceCut2DTransformation>
        transformations = reach->createCutTransformations(cutXY, rootNode);
    ...
    VirtualRobot::WorkspaceGridPtr reachGrid(new VirtualRobot::WorkspaceGrid(
        minBB(0), maxBB(0), minBB(1), maxBB(1), reach->getDiscretizeParameterTranslation()));
    ...
    reach->getWorkspaceExtends(minBB, maxBB);
    reachGrid.reset(new VirtualRobot::WorkspaceGrid(
        minBB(0), maxBB(0), minBB(1), maxBB(1), reach->getDiscretizeParameterTranslation()));
