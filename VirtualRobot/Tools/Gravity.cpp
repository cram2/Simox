...
Gravity::Gravity(VirtualRobot::RobotPtr robot,
                 VirtualRobot::RobotNodeSetPtr rnsJoints,
                 VirtualRobot::RobotNodeSetPtr rnsBodies) :
    robot(robot), rns(rnsJoints), rnsBodies(rnsBodies)
{
    ...
    gravityDataHelperRoot =
        GravityData::create(robot->getRootNode(), nodes, nodesBodies, gravityDataHelperVec);
}

Gravity::~Gravity() = default;

void
Gravity::computeGravityTorque(std::vector<float>& storeValues)
{
...
Gravity::GravityData::GravityData() = default;

Gravity::GravityDataPtr
Gravity::GravityData::create(SceneObjectPtr node,
                             const std::vector<RobotNodePtr>& joints,
                             const std::vector<RobotNodePtr>& bodies,
                             std::vector<Gravity::GravityDataPtr>& dataVec)
{
...
void
Gravity::GravityData::init(SceneObjectPtr node,
                           const std::vector<RobotNodePtr>& joints,
                           const std::vector<RobotNodePtr>& bodies,
...
void
Gravity::GravityData::computeCoMAndTorque(Eigen::Vector3f& comPositionGlobal)
{
    ...
    if (computeTorque)
    {
        const VirtualRobot::RobotNodeRevolutePtr rnRevolute =
            std::dynamic_pointer_cast<VirtualRobot::RobotNodeRevolute>(node);
        const Eigen::Vector3f axisGlobal = rnRevolute->getJointRotationAxis();
        const VirtualRobot::MathTools::BaseLine<Eigen::Vector3f> l(
            node->getGlobalPose().block<3, 1>(0, 3), axisGlobal);
        const auto pointOnAxis =
            VirtualRobot::MathTools::nearestPointOnLine<Eigen::Vector3f>(l, comPositionGlobal);
        Eigen::Vector3f r =
            comPositionGlobal - pointOnAxis; // vector from axis to com (change sign?)
        r *= 0.001f; // mm -> m
