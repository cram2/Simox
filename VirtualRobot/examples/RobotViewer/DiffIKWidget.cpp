...
QDialog*
DiffIKWidget::getDialog()
{
    return dialog;
}

void
DiffIKWidget::open(QWidget* parent, SoSeparator* sceneSep)
{
    if (!diffIKWidget)
    {
...
void
DiffIKWidget::update(VirtualRobot::RobotPtr robot)
{
...
Eigen::MatrixXd
DiffIKWidget::readFollowManipulability()
{
    try
    {
...
VirtualRobot::AbstractManipulability::Mode
DiffIKWidget::getMode(QComboBox* comboBox)
{
...
VirtualRobot::AbstractManipulability::Type
DiffIKWidget::getManipulabilityType(QComboBox* comboBox)
{
...
Eigen::Matrix4f
DiffIKWidget::getEndEffectorPos()
{
...
Eigen::Matrix4f
DiffIKWidget::getEndEffectorPos2()
{
...
void
DiffIKWidget::updateEndeffectorPoseVis(VirtualRobot::RobotNodeSetPtr robotNodeSet,
    ...
    if (robotNodeSet)
    {
        ...
        if (tcp)
        {
            ...
            if (ui->checkBoxOriIK->isChecked())
            {
                ...
                {
                    sep->addChild(
                        VirtualRobot::CoinVisualizationFactory::CreateSphere(50, Color::Red()));
                }
            }
            else
            {
...
VirtualRobot::AbstractManipulabilityTrackingPtr
DiffIKWidget::getManipulabilityTracking()
{
...
void
DiffIKWidget::updateDistance(double distance)
{
    ui->currentDistance->setText(QString::fromStdString(std::to_string(distance)));
}

void
DiffIKWidget::updateJointValues(const std::map<std::string, float>& jointValues)
{
...
void
DiffIKWidget::solveIK(bool untilReached)
{
    ...
    if (ui->checkBoxSoechtingNullspace->isChecked() && kSoechting > 0)
    {
        if (robot->getName() == "Armar6" && currentRobotNodeSet->getName() == "RightArm")
        {
            ...
            VirtualRobot::SoechtingNullspaceGradientPtr gradient(
                new VirtualRobot::SoechtingNullspaceGradient(
...
void
DiffIKWidget::setEndEffectorPose()
{
    if (currentRobotNodeSet && currentRobotNodeSet->getTCP())
    {
        auto tcp = currentRobotNodeSet->getTCP();
...
void
DiffIKWidget::setRandomJointValues()
{
    robot->setPropagatingJointValuesEnabled(false);
    std::vector<std::string> robotNodeNames = currentRobotNodeSet->getNodeNames();
...
void
DiffIKWidget::setRobotNodeSet(QString name)
{
    currentRobotNodeSet = robot->getRobotNodeSet(name.toStdString());
}

void
DiffIKWidget::setRobotNodeSet2(QString name)
{
    currentRobotNodeSet2 = robot->getRobotNodeSet(name.toStdString());
}

void
printJacobi(VirtualRobot::RobotNodeSetPtr rns, bool scale)
{
    if (!rns)
        return;
...
void
DiffIKWidget::printJacobian()
{
    printJacobi(currentRobotNodeSet, true);
}

void
DiffIKWidget::printJacobian2()
{
    printJacobi(currentRobotNodeSet, true);
}

void
Worker::followManip(VirtualRobot::AbstractManipulabilityTrackingPtr manipTracking,
...
void
Worker::solveIK(VirtualRobot::CompositeDiffIKPtr ik,
                int steps,
