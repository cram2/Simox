...
namespace VirtualRobot
{
    ...
    ChainedGrasp::ChainedGrasp(const std::string& name,
    ...
    void
    ChainedGrasp::print(bool printDecoration) const
    {
    ...
    RobotNodePtr
    ChainedGrasp::attachChain(RobotPtr robot,
                              GraspableSensorizedObjectPtr object,
                              bool addObjectVisualization)
    {
        if (!robot || !object)
        {
            return nullptr;
    ...
    void
    ChainedGrasp::updateChain(RobotPtr robot)
    {
        if (!robot)
        {
            return;
    ...
    void
    ChainedGrasp::detachChain(RobotPtr robot)
    {
    ...
    RobotNodePtr
    ChainedGrasp::getVirtualNode(RobotPtr robot, const std::string& virtualName)
    {
        if (robot)
        {
    ...
    RobotNodePtr
    ChainedGrasp::getObjectNode(RobotPtr robot)
    {
        return getVirtualNode(robot, "object");
    }

    RobotNodePtr
    ChainedGrasp::attachChain(RobotNodePtr robotNode,
                              GraspableSensorizedObjectPtr object,
                              bool addObjectVisualization)
    {
        ...
        RobotNodePtr virtual_object(
            new RobotNodeFixed(robot,
                               ROBOT_NODE_NAME("object"),
                               graspableObjectCoordSysTransformation,
                               addObjectVisualization ? object->getVisualization() : nullptr,
                               addObjectVisualization ? object->getCollisionModel() : nullptr,
                               object->getPhysics(),
    ...
    RobotNodePtr
    ChainedGrasp::attach(VirtualJoint joint, RobotNodePtr robotNode)
    {
        auto robot = robotNode->getRobot();
        RobotNodePtr virtualNode;
        if (joint.isRevolute)
        {
            virtualNode.reset(new RobotNodeRevolute(robot,
                                                    ROBOT_NODE_NAME(joint.name),
                                                    joint.min,
                                                    joint.max,
        ...
        {
            virtualNode.reset(new RobotNodePrismatic(robot,
                                                     ROBOT_NODE_NAME(joint.name),
                                                     joint.min,
                                                     joint.max,
                                                     Eigen::Matrix4f::Identity(),
    ...
    bool
    ChainedGrasp::update(VirtualJoint joint, RobotPtr robot)
    {
    ...
    void
    ChainedGrasp::sampleGraspsUniform(std::vector<Eigen::Matrix4f>& grasps,
                                      RobotPtr robot,
                                      unsigned int grid)
    {
        ...
        for (unsigned int i = 0; i < joint_grid; i++)
        {
            if (children.empty())
            {
                ...
                Eigen::Matrix4f localPose =
                    robotNode->getRobot()->getRobotNode(rootName)->toLocalCoordinateSystem(
                        globalPose);
            ...
            else
            {
                ...
                sampleGraspsUniform(
                    grasps, rootName, std::dynamic_pointer_cast<RobotNode>(children[0]), grid);
            }
        }
    }

    Eigen::Matrix4f
    ChainedGrasp::getLocalPose() const
    {
        return simox::math::pos_rpy_to_mat4f(getPositionXYZ(), getOrientationRPY());
    }

    std::vector<RobotPtr>
    ChainedGrasp::sampleHandsUniform(RobotPtr robot, unsigned int grid)
    {
        ...
        if (virtualObject && endEffector)
        {
            ...
            sampleGraspsUniform(
                ...
                robot->clone("clonedHand")->getEndEffector(endEffector->getName())->getTcp(),
                grid);
    ...
    ChainedGrasp::VirtualJoint*
    ChainedGrasp::getVirtualJoint(const std::string& name)
    {
        if (name == x.name)
            return &x;
        else if (name == y.name)
            ...
            return &z;
        else if (name == roll.name)
            return &roll;
        else if (name == pitch.name)
            return &pitch;
        else if (name == yaw.name)
            return &yaw;
        else
        {
    ...
    std::vector<float>
    ChainedGrasp::getUsedVirtualLimits() const
    {
    ...
    Eigen::Vector3f
    ChainedGrasp::getPositionXYZ() const
    {
        return Eigen::Vector3f(x.value, y.value, z.value);
    }

    Eigen::Vector3f
    ChainedGrasp::getOrientationRPY() const
    {
        return Eigen::Vector3f(roll.value, pitch.value, yaw.value);
    }

    Eigen::Matrix4f
    ChainedGrasp::getTransformation() const
    {
        return getLocalPose() * graspableObjectCoordSysTransformation;
    }

    Eigen::Matrix4f
    ChainedGrasp::getLocalPoseGrasp()
    {
        return getTransformation().inverse();
    }

    void
    ChainedGrasp::setObjectTransformation(
        const Eigen::Matrix4f& graspableObjectCoordSysTransformation)
    {
        this->graspableObjectCoordSysTransformation = graspableObjectCoordSysTransformation;
    }

    std::string
    ChainedGrasp::getTransformationXML(const std::string& tabs) const
    {
        std::stringstream ss;
        std::string tt = tabs + "\t";
        std::string ttt = tabs + "\t\t";
    ...
    bool
    ChainedGrasp::visualizeRotationCoordSystem(RobotPtr robot, bool visible)
    {
    ...
    std::string
    ChainedGrasp::ROBOT_NODE_NAME(const std::string& virtualJointName)
    {
        std::stringstream ss;
        ss << "virtual_";
        ss << eef;
    ...
    EndEffectorPtr
    ChainedGrasp::getEndEffector(RobotPtr robot)
    {
        if (robot->hasEndEffector(eef))
        {
    ...
    RobotNodePtr
    ChainedGrasp::getTCP(RobotPtr robot)
    {
        auto endEffector = getEndEffector(robot);
        if (endEffector)
        {
    ...
    ChainedGrasp::getNames(bool onlyAdaptable)
    {
        std::vector<std::string> names;
        if (x.isUsed() || !onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME(x.name));
        if (y.isUsed() || !onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME(y.name));
        if (z.isUsed() || !onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME(z.name));
        if (roll.isUsed() || !onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME(roll.name));
        if (pitch.isUsed() || !onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME(pitch.name));
        if (yaw.isUsed() || !onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME(yaw.name));
        if (!onlyAdaptable)
            names.push_back(ROBOT_NODE_NAME("object"));
        return names;
    }

    RobotNodeSetPtr
    ChainedGrasp::createRobotNodeSet(RobotPtr robot, RobotNodeSetPtr rns)
    {
        auto virtualObject = getObjectNode(robot);
        if (virtualObject)
        {
            std::vector<std::string> nodeNames;
            for (const std::string& name : rns->getNodeNames())
                nodeNames.push_back(name);
            //int index = nodeNames.size();
            for (const std::string& name : getNames(true))
                nodeNames.push_back(name);
        ...
        while (node != robot->getRobotNode(nodeNames.at(index-1))) {
            ...
            return RobotNodeSet::createRobotNodeSet(robot,
                                                    this->name,
                                                    nodeNames,
                                                    rns->getKinematicRoot()->getName(),
                                                    virtualObject->getName());
        }
        return nullptr;
    }
    ...
    bool
    ChainedGrasp::VirtualJoint::setValue(float value)
    {
    ...
    void
    ChainedGrasp::VirtualJoint::resetLimits()
    {
        min = value;
        max = value;
    }
    ...
    bool
    ChainedGrasp::VirtualJoint::setLimitsValue(float min, float max)
    {
    ...
    bool
    ChainedGrasp::VirtualJoint::setLimits(float min, float max)
    {
        if (min <= max)
        {
            if (isRevolute)
    ...
    bool
    ChainedGrasp::VirtualJoint::setLimitless()
    {
        if (isRevolute)
        {
            this->min = -M_PI;
    ...
    bool
    ChainedGrasp::VirtualJoint::isLimitless()
    {
        return max - min > 2 * M_PI - 0.000001;
    }

    Eigen::Vector3f
    ChainedGrasp::VirtualJoint::getAxis() const
    {
    ...
    float
    ChainedGrasp::VirtualJoint::getMin() const
    {
        return min;
    }

    float
    ChainedGrasp::VirtualJoint::getMax() const
    {
        return max;
    }

    float
    ChainedGrasp::VirtualJoint::isUsed() const
    {
        return std::abs(max - min) > 0.000001;
    }

    std::string
    ChainedGrasp::VirtualJoint::toXML() const
    {
        std::stringstream ss;
    ...
    ChainedGrasp::VirtualJoint::VirtualJoint(const std::string& name, bool isRevolute, int axis) :
        name(name), value(0.0f), min(0.0f), max(0.0f), isRevolute(isRevolute), axis(axis)
    {
    }

    void
    ChainedGrasp::VirtualJoint::addLimits(std::vector<float>& usedLimits) const
    {
        if (isUsed())
        {
