...
ContactPair
ContactSection::addPair(const Geom& geom1, const Geom& geom2)
{
    return addPair(geom1.name.get(), geom2.name.get());
}

ContactPair
ContactSection::addPair(const std::string& geom1Name, const std::string& geom2Name)
{
...
ContactExclude
ContactSection::addExclude(const Body& body1, const Body& body2)
{
    return addExclude(body1.name.get(), body2.name.get());
}

ContactExclude
ContactSection::addExclude(const std::string& body1Name, const std::string& body2Name)
{
