...
namespace VirtualRobot
{
    ...
    void
    TriMeshUtils::CreateBoxTriangles(std::vector<TriMeshModel::triangle>& triangles,
                                     ...
                                     float depth)
    {
        ...
        auto transf = [&](Eigen::Vector3f& pos) { pos = globalOrientation * pos + globalPos; };
    ...
    std::vector<TriMeshModel::triangle>
    TriMeshUtils::CreateBoxTriangles(const Eigen::Matrix4f& globalPose,
                                     float width,
                                     float height,
                                     float depth)
    {
    ...
    TriMeshModelPtr
    TriMeshUtils::CreateBox(const Eigen::Matrix4f& globalPose,
                            float width,
                            float height,
                            float depth,
                            ...
                            const std::vector<VisualizationFactory::Color>& colors)
    {
        ...
        for (auto& t : triangles)
        {
            ...
            mesh->addTriangleWithFace(t.vertex1,
                                      t.vertex2,
                                      t.vertex3,
    ...
    TriMeshModelPtr
    TriMeshUtils::CreateSparseBoxGrid(const Eigen::Matrix4f& globalPose,
    ...
    TriMeshUtils::sampleSurfacePoint(const std::vector<float>& cumulativeAreas,
                                     double totalArea,
                                     const TriMeshModel& tri)
    {
        if (tri.faces.empty())
        {
            return std::nullopt;
    ...
    TriMeshUtils::uniformSampling(const TriMeshModel& tri, unsigned int n)
    {
        ...
        while (i < n)
        {
            const std::optional<Eigen::Vector3f> sampledPoint =
                sampleSurfacePoint(cumulativeAreas, totalArea, tri);
