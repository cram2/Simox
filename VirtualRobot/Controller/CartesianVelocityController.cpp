...
namespace VirtualRobot
{
    ...
    CartesianVelocityController::CartesianVelocityController(
        ...
        const JacobiProvider::InverseJacobiMethod invJacMethod,
    ...
    void
    CartesianVelocityController::calculateJacobis(IKSolver::CartesianSelection mode)
    {
        ...
        _inv = ik->computePseudoInverseJacobianMatrix(_jacobiWithCosts,
 ...
 */
        ...
        const Eigen::VectorXf& margins)
    {
        ...
        for (size_t i = 0; i < rns->getSize(); i++)
        {
            ...
            else
            {
                ...
                r(i) = math::Helpers::Clamp(
                           0.f, 1.f, math::Helpers::ILerp(lo + mrg, lo, rn->getJointValue())) +
                       math::Helpers::Clamp(
                           0.f, 1.f, math::Helpers::ILerp(hi, hi - mrg, rn->getJointValue()));
            }
        }
        return r;
    }
    ...
    Eigen::VectorXf
    CartesianVelocityController::calculateNullspaceVelocity(const Eigen::VectorXf& cartesianVel,
                                                            float KpScale,
                                                            IKSolver::CartesianSelection mode)
    {
    ...
    Eigen::VectorXf
    CartesianVelocityController::calculateRegularization(IKSolver::CartesianSelection mode)
    {
    ...
    bool
    CartesianVelocityController::clampJacobiAtJointLimits(IKSolver::CartesianSelection mode,
        ...
        for (size_t i = 0; i < size; ++i)
        {
            ...
            if ((node->getJointValue() >= node->getJointLimitHigh() - jointLimitCheckAccuracy &&
                ...
                (node->getJointValue() <= node->getJointLimitLow() + jointLimitCheckAccuracy &&
                 jointVel(i) < 0))
            {
                for (int k = 0; k < jacobi.rows();
                     ++k) // memory allocation free resetting of column
                {
                    jacobi(k, i) = 0.0f;
                }
                modifiedJacobi = true;
