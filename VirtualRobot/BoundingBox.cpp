...
namespace VirtualRobot
{
    ...
    bool
    BoundingBox::planeGoesThrough(const MathTools::Plane& p)
    {
        bool left = false;
        bool right = false;
    ...
    BoundingBox::getPoints() const
    {
        ...
        if (std::isnan(min(0)) || std::isnan(min(1)) || std::isnan(min(2)) || std::isnan(max(0)) ||
            std::isnan(max(1)) || std::isnan(max(2)))
        {
            return points;
        }
    ...
    void
    BoundingBox::addPoints(const std::vector<Eigen::Vector3f>& p)
    {
    ...
    void
    BoundingBox::addPoints(const BoundingBox& bbox)
    {
    ...
    void
    BoundingBox::addPoint(const Eigen::Vector3f& p)
    {
        for (int j = 0; j < 3; j++)
        {
    ...
    Eigen::Vector3f
    BoundingBox::getMax() const
    {
        return max;
    }

    Eigen::Vector3f
    BoundingBox::getMin() const
    {
        return min;
    }
    ...
    void
    BoundingBox::transform(Eigen::Matrix4f& pose)
    {
        if (std::isnan(min(0)) || std::isnan(min(1)) || std::isnan(min(2)) || std::isnan(max(0)) ||
            std::isnan(max(1)) || std::isnan(max(2)))
        {
    ...
    void
    BoundingBox::scale(const Eigen::Vector3f& scaleFactor)
    {
        if (std::isnan(min(0)) || std::isnan(min(1)) || std::isnan(min(2)) || std::isnan(max(0)) ||
            std::isnan(max(1)) || std::isnan(max(2)))
        {
    ...
    std::string
    BoundingBox::toXML(int tabs, bool skipMatrixTag)
    {
        std::stringstream ss;
        std::string t;
    ...
    bool
    BoundingBox::isInside(Eigen::Vector3f point) const
    {
