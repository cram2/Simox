...
namespace VirtualRobot
{
    ...
    BimanualManipulability::BimanualManipulability(const RobotNodeSetPtr& rnsLeft,
                                                   const RobotNodeSetPtr& rnsRight,
                                                   Mode mode,
                                                   Type type,
                                                   bool convertMMtoM) :
        BimanualManipulability(rnsLeft,
                               rnsRight,
                               rnsLeft->getTCP(),
                               rnsRight->getTCP(),
                               mode,
    ...
    BimanualManipulability::BimanualManipulability(const RobotNodeSetPtr& rnsLeft,
                                                   const RobotNodeSetPtr& rnsRight,
                                                   const RobotNodePtr& nodeLeft,
                                                   const RobotNodePtr& nodeRight,
                                                   Mode mode,
                                                   Type type,
                                                   const SceneObjectPtr& object,
                                                   const RobotNodePtr& coordSystem,
                                                   bool convertMMtoM) :
        AbstractManipulability(mode, type),
        rnsLeft(rnsLeft),
        rnsRight(rnsRight),
        nodeLeft(nodeLeft),
        nodeRight(nodeRight),
        ...
        coordSystem(coordSystem)
    {
        ...
        {
            throw VirtualRobotException("Left and right robot node set of bimanual manipulability "
                                        "do not map to the same robot!");
        }
    ...
    Eigen::MatrixXd
    BimanualManipulability::computeFullJacobianLeft()
    {
        return computeJacobianLeft(IKSolver::All);
    }

    Eigen::MatrixXd
    BimanualManipulability::computeFullJacobianRight()
    {
        return computeJacobianRight(IKSolver::All);
    }

    Eigen::MatrixXd
    BimanualManipulability::computeBimanualJacobian(const Eigen::MatrixXd& jacobianLeft,
                                                    const Eigen::MatrixXd& jacobianRight)
    {
        ...
        Eigen::MatrixXd jacobianBimanual =
            Eigen::MatrixXd::Zero(nbVarsLeft + nbVarsRight, nbDofsLeft + nbDofsRight);
    ...
    Eigen::MatrixXd
    BimanualManipulability::computeManipulability()
    {
        return computeManipulability(computeFullJacobian());
    }

    Eigen::MatrixXd
    BimanualManipulability::computeManipulability(const Eigen::MatrixXd& jacobian, Type type)
    {
        if (jacobian.rows() != 6)
            std::runtime_error(
                "computeManipulability(...) of Bimanual Maniulability requires the full jacobian!");
        return getManipulabilitySubMatrix(
            computeBimanualManipulability(jacobian, computeBimanualGraspMatrix(), type));
    }

    Eigen::Vector3f
    BimanualManipulability::getLocalPosition()
    {
        return rnsLeft->getRobot()->toGlobalCoordinateSystemVec(getGlobalPosition());
    }

    Eigen::Vector3f
    BimanualManipulability::getGlobalPosition()
    {
        if (object)
            return object->getCoMGlobal();
        else
            return (nodeLeft->getGlobalPosition() + nodeRight->getGlobalPosition()) / 2.0f;
    }

    Eigen::Matrix4f
    BimanualManipulability::getCoordinateSystem()
    {
        return coordSystem ? coordSystem->getGlobalPose() : Eigen::Matrix4f::Identity();
    }

    Eigen::MatrixXd
    BimanualManipulability::computeBimanualGraspMatrix()
    {
        ...
        Eigen::MatrixXd bimanualGraspMatrix =
            computeBimanualGraspMatrix(endeffectorLeft, endeffectorRight, rotation);
        return bimanualGraspMatrix;
    }

    RobotNodeSetPtr
    BimanualManipulability::getLeftRobotNodeSet()
    {
        return rnsLeft;
    }

    RobotNodeSetPtr
    BimanualManipulability::getRightRobotNodeSet()
    {
        return rnsRight;
    }

    std::vector<std::string>
    BimanualManipulability::getJointNames()
    {
        std::vector<std::string> robotNodeNames = rnsLeft->getNodeNames();
        auto rightRobotNodeNames = rnsRight->getNodeNames();
        robotNodeNames.insert(
            robotNodeNames.end(), rightRobotNodeNames.begin(), rightRobotNodeNames.end());
        return robotNodeNames;
    }

    Eigen::VectorXd
    BimanualManipulability::getJointAngles()
    {
    ...
    Eigen::VectorXd
    BimanualManipulability::getJointLimitsHigh()
    {
    ...
    Eigen::VectorXd
    BimanualManipulability::getJointLimitsLow()
    {
    ...
    RobotPtr
    BimanualManipulability::getRobot()
    {
        return rnsLeft->getRobot();
    }

    RobotNodeSetPtr
    BimanualManipulability::createRobotNodeSet(const std::string& name)
    {
        return RobotNodeSet::createRobotNodeSet(
            getRobot(), name, getJointNames(), getRobot()->getRootNode()->getName());
    }

    void
    BimanualManipulability::setConvertMMtoM(bool value)
    {
    ...
    Eigen::MatrixXd
    BimanualManipulability::computeBimanualManipulability(
        const Eigen::MatrixXd& bimanualJacobian,
        ...
        Type type)
    {
        ...
        Eigen::MatrixXd pinverseBimanualGraspMatrix =
            bimanualGraspMatrix.transpose() *
            (bimanualGraspMatrix * bimanualGraspMatrix.transpose()).inverse();
        // Compute manipulability
        Eigen::MatrixXd velocityManipulability = pinverseBimanualGraspMatrix.transpose() *
                                                 (bimanualJacobian * bimanualJacobian.transpose()) *
                                                 pinverseBimanualGraspMatrix;
    ...
    Eigen::MatrixXd
    BimanualManipulability::computeJacobian(IKSolver::CartesianSelection mode)
    {
        return computeBimanualJacobian(computeJacobianLeft(mode), computeJacobianRight(mode));
    }

    Eigen::MatrixXd
    BimanualManipulability::computeJacobianLeft(IKSolver::CartesianSelection mode)
    {
        return ikLeft->getJacobianMatrix(nodeLeft, mode).cast<double>();
    }

    Eigen::MatrixXd
    BimanualManipulability::computeJacobianRight(IKSolver::CartesianSelection mode)
    {
    ...
    Eigen::MatrixXd
    BimanualManipulability::computeBimanualGraspMatrix(
        const Eigen::Vector3f& endeffectorLeftPosition,
        const Eigen::Vector3f& endeffectorRightPosition,
        const Eigen::MatrixXd& rotation)
    {
        Eigen::Vector3f objectPosition = getLocalPosition();
        // Compute left and right grasp matrices
        Eigen::MatrixXd graspMatrixLeft =
            computeGraspMatrix(endeffectorLeftPosition, objectPosition, rotation);
        Eigen::MatrixXd graspMatrixRight =
            computeGraspMatrix(endeffectorRightPosition, objectPosition, rotation);
    ...
    Eigen::MatrixXd
    BimanualManipulability::computeGraspMatrix(const Eigen::VectorXf& endeffectorPosition,
                                               const Eigen::VectorXf& objectPosition,
                                               const Eigen::MatrixXd& rotation)
    {
