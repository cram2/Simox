...
CoMConstraint::CoMConstraint(const RobotPtr& robot,
...
void
CoMConstraint::updateTarget(const Eigen::Vector3f& target)
{
    ik->setGoal(target);
}

void
CoMConstraint::updateTarget(const Eigen::Vector2f& target)
{
    ik->setGoal(target);
}

double
CoMConstraint::optimizationFunction(unsigned int /*id*/)
{
    if (dimensions == 2)
    {
...
Eigen::VectorXf
CoMConstraint::optimizationGradient(unsigned int /*id*/)
{
