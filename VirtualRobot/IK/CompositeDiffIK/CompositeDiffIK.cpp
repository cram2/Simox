...
namespace VirtualRobot
{
    ...
    CompositeDiffIK::CompositeDiffIK(const RobotNodeSetPtr& rns) : rns(rns), robot(rns->getRobot())
    {
        ik.reset(new DifferentialIK(
            rns, rns->getRobot()->getRootNode(), JacobiProvider::eSVDDampedDynamic));
    }
    ...
    CompositeDiffIK::addTarget(const RobotNodePtr& tcp,
    ...
    CompositeDiffIK::addNullspacePositionTarget(const RobotNodePtr& tcp,
                                                const Eigen::Vector3f& target)
    {
        CompositeDiffIK::NullspaceTargetPtr nst(
            new CompositeDiffIK::NullspaceTarget(rns, tcp, target));
    ...
    CompositeDiffIK::Result
    CompositeDiffIK::solve(Parameters params, SolveState& s)
    {
    ...
    CompositeDiffIK::step(CompositeDiffIK::Parameters& params, SolveState& s, int stepNr)
    {
        ...
        if (s.cols > s.rows)
        {
            ...
            {
                Eigen::VectorXf nsgrad =
                    nsGradient->kP * nsGradient->getGradientAdjusted(params, stepNr);
                nullspaceVel += nsgrad;
            }

            //LimitInfNormTo(nullspaceVel, params.maxJointAngleStep);

            Eigen::JacobiSVD<Eigen::MatrixXf> svd(s.jacobi,
    ...
    int
    CompositeDiffIK::CartesianSelectionToSize(IKSolver::CartesianSelection mode)
    {
        switch (mode)
        {
    ...
    RobotNodeSetPtr
    CompositeDiffIK::getRobotNodeSet()
    {
        return rns;
    }

    RobotPtr
    CompositeDiffIK::getRobot()
    {
        return robot;
    }

    CompositeDiffIK::Target::Target(const RobotNodePtr& tcp,
                                    const Eigen::Matrix4f& target,
                                    IKSolver::CartesianSelection mode) :
    ...
    CompositeDiffIK::NullspaceTarget::NullspaceTarget(const RobotNodeSetPtr& rns,
                                                      const RobotNodePtr& tcp,
                                                      const Eigen::Matrix4f& target,
                                                      IKSolver::CartesianSelection mode) :
        NullspaceGradient(rns->getNodeNames()),
        tcp(tcp),
        target(target),
        mode(mode),
        pCtrl(tcp),
    ...
    CompositeDiffIK::NullspaceTarget::NullspaceTarget(const RobotNodeSetPtr& rns,
                                                      const RobotNodePtr& tcp,
                                                      const Eigen::Vector3f& target) :
        CompositeDiffIK::NullspaceTarget(
            rns,
            tcp,
    ...
    Eigen::VectorXf
    CompositeDiffIK::NullspaceTarget::getGradient(Parameters& params, int /*stepNr*/)
    {
    ...
    CompositeDiffIK::AdaptableNullspaceTarget::AdaptableNullspaceTarget(
        ...
        int startStepNr,
        int endStepNr) :
        NullspaceTarget(rns, tcp, target, mode), startStepNr(startStepNr), endStepNr(endStepNr)
    {
    ...
    CompositeDiffIK::AdaptableNullspaceTarget::AdaptableNullspaceTarget(
        ...
        int startStepNr,
        int endStepNr) :
        NullspaceTarget(rns, tcp, target), startStepNr(startStepNr), endStepNr(endStepNr)
    {
        assert(startStepNr < endStepNr);
    }

    Eigen::VectorXf
    CompositeDiffIK::AdaptableNullspaceTarget::getGradient(Parameters& params, int stepNr)
    {
        Eigen::VectorXf gradient = NullspaceTarget::getGradient(params, stepNr);
        float factor = 0;
        if ((int)params.steps < (endStepNr - startStepNr) ||
            stepNr >= (int)params.steps - startStepNr)
            factor = 1;
        else if (stepNr >= startStepNr)
        {
            factor =
                ((float)(stepNr - startStepNr)) / ((int)params.steps - (endStepNr - startStepNr));
        }
        return gradient * factor;
    }

    Eigen::VectorXf
    CompositeDiffIK::LimitInfNormTo(Eigen::VectorXf vec,
                                    float maxValue,
    ...
    CompositeDiffIK::NullspaceJointTarget::NullspaceJointTarget(const RobotNodeSetPtr& rns) :
        NullspaceGradient(rns->getNodeNames()),
        rns(rns),
        target(rns->getJointValuesEigen()),
        weight(Eigen::VectorXf::Zero(rns->getSize()))
    {
    }
    ...
    void
    CompositeDiffIK::NullspaceJointTarget::set(const std::string& jointName,
                                               float target,
                                               float weight)
    {
    ...
    void
    CompositeDiffIK::NullspaceJointTarget::set(const RobotNodePtr& rn, float target, float weight)
    {
    ...
    Eigen::VectorXf
    CompositeDiffIK::NullspaceJointTarget::getGradient(Parameters& /*params*/, int /*stepNr*/)
    {
    ...
    CompositeDiffIK::NullspaceJointLimitAvoidance::NullspaceJointLimitAvoidance(
        ...
        NullspaceGradient(rns->getNodeNames()),
        rns(rns),
        weight(Eigen::VectorXf::Constant(rns->getSize(), 1))
    {
    }
    ...
    void
    CompositeDiffIK::NullspaceJointLimitAvoidance::setWeight(const std::string& jointName,
                                                             float weight)
    {
        int index = rns->getRobotNodeIndex(jointName);
        if (index < 0)
    ...
    void
    CompositeDiffIK::NullspaceJointLimitAvoidance::setWeight(const RobotNodePtr& rn, float weight)
    {
        int index = rns->getRobotNodeIndex(rn);
        if (index < 0)
        {
    ...
    void
    CompositeDiffIK::NullspaceJointLimitAvoidance::setWeights(const RobotNodeSetPtr& rns,
                                                              float weight)
    {
    ...
    CompositeDiffIK::NullspaceGradient::NullspaceGradient(
    ...
    Eigen::VectorXf
    CompositeDiffIK::NullspaceGradient::getGradientAdjusted(CompositeDiffIK::Parameters& params,
                                                            int stepNr)
    {
    ...
    void
    CompositeDiffIK::NullspaceGradient::setMapping(const RobotNodeSetPtr& rns)
    {
        ikSize = rns->getSize();
    ...
    std::map<int, int>
    CompositeDiffIK::NullspaceGradient::getMapping(const RobotNodeSetPtr& rns)
    {
        ...
        if (this->jointNames != rns->getNodeNames())
        {
            for (unsigned int i = 0; i < jointNames.size(); i++)
            {
                ...
                if (index >= 0)
                {
                    ...
                    std::cout << "Ignoring null space node " << jointNames.at(i)
                              << " which is not part of ik" << std::endl;
            }
        }
        return mapping;
    }
    ...
    CompositeDiffIK::NullspaceGradient::getJointNames() const
    {
        return jointNames;
    }
