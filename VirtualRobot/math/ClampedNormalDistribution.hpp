...
namespace VirtualRobot
{
    ...
    public:
        ...
        static ClampedNormalDistribution
        Unclamped(RealType mean = 0, RealType stddev = 1)
        {
            return ClampedNormalDistribution(mean,
                                             stddev,
        ...
        mean() const
        {
            return _distrib.mean();
        }

        /// Set the mean of the normal distribution.
        void
        setMean(RealType value)
        {
            this->_distrib = {value, _distrib.stddev()};
        }
        ...
        stddev() const
        {
            return _distrib.stddev();
        }

        /// Set the standard deviation of the normal distribution.
        void
        setStddev(RealType value)
        {
            this->_distrib = {_distrib.mean(), value};
        }
        ...
        bool
        operator==(const ClampedNormalDistribution& rhs) const
        {
            return this == &rhs || (this->mean() == rhs.mean() && this->stddev() == rhs.stddev() &&
                                    this->_min == rhs._min && this->_max == rhs._max &&
    ...
    std::ostream&
    operator<<(std::ostream& os, const ClampedNormalDistribution<RealT>& rhs)
    {
        // for some reason, the first string must be wrapped in string constructor
        os << std::string("<ClampedNormalDistribution mean=") << rhs.mean()
           << " stddev=" << rhs.stddev() << " [min, max]=[" << rhs.min() << ", " << rhs.max()
           << "] >";
        return os;
    }
